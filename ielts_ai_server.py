"""
IELTS Phạm Tiến Dũng Gia Sư Proxy Server
...
# Directory for Phạm Tiến Dũng Gia Sư Output in the writing app
WRITING_APP_DATA_DIR = r"d:\ietls_wrrting\data\ai_output"
os.makedirs(WRITING_APP_DATA_DIR, exist_ok=True)

# Speaking project Phạm Tiến Dũng Gia Sư vocab directory (served directly by http.server)
...
def add_to_queue(task_type, data):
    ...
    try:
        ...
        print(f"[DB ERROR] Add to queue failed: {e}")
        return None
...
async def process_queue_worker():
    ...
    while True:
        ...
        try:
            ...
            if task:
                ...
                try:
                    if task['task_type'] == "barem":
                        ...
                        # Fetch existing words to avoid duplication
                        existing_words = []
                        vocab_dir = os.path.join(WRITING_APP_DATA_DIR, "vocab")
                        clean_qid = str(data["id"]).replace(" ", "_").replace("/", "-")
                        vocab_type = data.get("vocab_type", "vocabulary")
                        master_path = os.path.join(vocab_dir, f"{clean_qid}_vocab_{vocab_type}.json")
                        if os.path.exists(master_path):
                            try:
                                with open(master_path, "r", encoding="utf-8") as f:
                                    existing_data = json.load(f)
                                    existing_words = [v["word"] for v in existing_data.get("vocab", [])]
                            except: pass
                    ...
                    error_occurred = True
                    err_msg = str(e)
                    print(f"[ERROR] Task #{task_id} FPhạm Tiến Dũng Gia SưLED: {err_msg}")
            ...
            print(f"[DB ERROR] Worker loop error: {e}")
            await asyncio.sleep(10)
...
def save_barem_to_file(qid, band, content):
    ...
    clean_qid = str(qid).replace(" ", "_").replace("/", "-")
    filename = f"{clean_qid}_band{band}.json"
    data = {"id": qid, "band": band, "content": content, "timestamp": datetime.datetime.now().isoformat(), "source": "Phạm Tiến Dũng Gia Sư"}
...
async def generate_vocab_for_question(question, count, vocab_type="vocabulary", chunk=1, avoid_words=None):
    global gemini_chat
    ...
    avoid_str = f"\nTUYỆT ĐỐI TRÁNH các từ/cụm từ sau đây (đã có): {', '.join(avoid_words)}" if avoid_words else ""
...
async def analyze_recordings(recordings: list) -> dict:
    ...
    try:
        ...
        if not response or not response.text:
            return {"error": "Phạm Tiến Dũng Gia Sư returned an empty response"}
        ...
        if not isinstance(data, dict):
            ...
            return {"error": "Phạm Tiến Dũng Gia Sư response is not a JSON object", "raw": raw_txt}
        ...
        print(f"Phạm Tiến Dũng Gia Sư Process Error: {e}\nRaw Output: {raw_output[:500]}...")
        return {"error": f"Phạm Tiến Dũng Gia Sư Process Error: {str(e)}", "raw": raw_output}
...
async def lookup_word(word: str) -> dict:
    ...
    print(f"[Phạm Tiến Dũng Gia Sư LOOKUP] {word_clean} (Querying Gemini...)")
    ...
    try:
        ...
        print(f"[Phạm Tiến Dũng Gia Sư ERROR] JSON parsing failed: {e}")
...
def run_server():
    ...
    print("=" * 55)
    print("  IELTS Phạm Tiến Dũng Gia Sư Proxy Server")
